...
ax.plot(baseline_step_data, baseline_prob_data, color='#F92816', alpha=0.2, marker='o', markersize=0.5, linestyle='', label="baseline method")
label_baseline, = ax.plot([-1000_0000],[0], color='#F92816', marker='o', linestyle='', label="baseline method")
# plot data of our method
# green : 0CC94B or 079C29, blue : 091DF4 or 040EC6
ax.plot(probbbc_step_data, probbbc_prob_data, color='#091DF4', alpha=0.6, marker='o', markersize=0.5, linestyle='', label="our method")
label_probbbc, = ax.plot([-1000_0000],[0], color='#091DF4', marker='o', linestyle='', label="our method")
# plot true value
...
if xtarget:
  ...
  if uniform_smc_file:
    ...
    ax.boxplot([probbbc_data, baseline_data, uniform_data], widths=[.5,.5,.5])
    ax.set_xticklabels(['ProbBBC', 'baseline', 'uniform'])
  else:
    ax.boxplot([probbbc_data, baseline_data], widths=[.5,.5])
  ...
  fig.savefig(os.path.splitext(output_file)[0] + '_boxplot.eps', bbox_inches="tight", pad_inches=0.05)
